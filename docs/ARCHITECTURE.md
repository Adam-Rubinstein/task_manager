...
- В `schema.sql` в `tasks` есть обязательное поле `title VARCHAR(255) NOT NULL`, но в текущей JPA-модели `Task` отдельного поля `title` нет (заголовок вычисляется из первой строки `description`). Это нужно синхронизировать: либо добавить `title` в сущность, либо менять схему/маппинг.
