...
- Типы оповещений (enum `AlertType`): `NOTIFICATION`, `WARNING`, `ERROR`, `DEADLINE_REMINDER`, `TASK_COMPLETED`

### 🔁 Повторяющиеся задачи (заготовка)

- Типы повтора (enum `RecurrenceType`): `NONE`, `DAILY`, `WEEKLY`, `MONTHLY`, `CUSTOM`
- Интервал повтора (для `CUSTOM`) - скрывается, если выбран "Без повтора"
- В `TaskService` реализован расчёт следующей даты повтора и создание следующей задачи
...
│   │   │       │   ├── 📄 AlertRepository.java       # 💾 Репозиторий оповещений
│   │   │       │   └── 📄 AudioFileRepository.java   # 💾 Репозиторий аудио
│   │   │       │
│   │   │       ├── 📁 model/
...
│   │
│   └── 📁 test/                                # 🧪 Тесты (пока не реализованы)
│
...
| Git        | Любая             | [Git SCM](https://git-scm.com/) |
...
- Email: [adam-rubinstein@mail.ru](mailto:adam-rubinstein@mail.ru)
